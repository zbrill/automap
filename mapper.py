...
class FieldMatch(dspy.Signature):
    ...
    target_fields: List[str] = dspy.InputField(desc="List of target field names to match against")
    ...
    explanation: str = dspy.OutputField(desc="Explanation of why they match semantically")
...
class BatchFieldMatches(dspy.Signature):
    ...
    source_fields: List[str] = dspy.InputField(desc="List of source field names to match")
    target_fields: List[str] = dspy.InputField(desc="List of target field names to match against")

    matches: List[FieldMatch] = dspy.OutputField(
        desc="Mapping of each source field to its match info (containing match, confidence, explanation)"
    )
...
class BatchFieldMatchPredictor(dspy.Module):
    ...
    def match(self, source_fields, target_fields):
        return self.batch_match(source_fields=source_fields, target_fields=target_fields)
...
class AutoMapper:
    ...
    def batch_match_fields(self, source_fields, target_fields, threshold=0.5):
        """
        ...
        for field, match_info in result.matches.items():
            match_dict = match_info
            if isinstance(match_info, str):
                # Handle case where LM might return a string instead of a dict
                try:
                    match_dict = json.loads(match_info)
            ...
            # if isinstance(match_dict["confidence"], str):
            #     try:
            #         match_dict["confidence"] = float(match_dict["confidence"])
            #     except ValueError:
            #         match_dict["confidence"] = 0.5

            # Apply threshold
            if any(match_dict[match].confidence < threshold for match in match_dict):
               match_dict["match"] = None
               match_dict["status"] = "rejected"
            else:
                match_dict["status"] = "accepted"

            filtered_matches[field] = match_dict
    ...
    def optimize_with_examples(self, example_pairs):
        ...
        self.optimized_single_matcher = optimizer.compile(
            FieldMatchPredictor(),
            trainset=train_data[:len(train_data)//2]
        )
    ...
    def _batch_accuracy_metric(self, example, prediction):
        ...
        for source_field, expected_match_info in example.matches.items():
            ...
            predicted_match = predicted_match_info.get("match") if isinstance(predicted_match_info, dict) else None
